...
class Window:
    ...
    def __init__(self):
        ...
        # Table for CSV file
        self.df = None
        self.csvTable = Table(self.labelFrameForCSVFile, dataframe=self.df, showstatusbar=True)
        self.csvTable.show();
        ...
        # Log for main frame
        self.mainLog = Text2(labelFrameForMainLog, width=self.width, height=self.height - 300)
    ...
    def displayPredictionWindow(self):
        ...
        vcmdForInt = (self.predictionWindow.register(self.validateInt2), '%d', '%i', '%P', '%s', '%S', '%v', '%V', '%W')
        vcmdForFloat = (
        self.predictionWindow.register(self.validateFloat), '%d', '%i', '%P', '%s', '%S', '%v', '%V', '%W')
        vcmdForFloat2 = (
        self.predictionWindow.register(self.validateFloat2), '%d', '%i', '%P', '%s', '%S', '%v', '%V', '%W')
        ...
        self.prediction_df = pd.DataFrame(columns=self.X.columns.values.tolist())
        self.predictionTable = Table(labelFrameForRightFrame, dataframe=self.prediction_df, showstatusbar=True)
    ...
    def selectedAlgorithm(self, algorithm):
        ...
        # %d = Type of action (1=insert, 0=delete, -1 for others)
        # %P = value of the entry if the edit is allowed (all, focusin, focusout, forced)
        vcmdForInt = (self.mainFrame.register(self.validateInt), '%d', '%i', '%P', '%s', '%S', '%v', '%V', '%W')
        vcmdForFloat = (self.mainFrame.register(self.validateFloat), '%d', '%i', '%P', '%s', '%S', '%v', '%V', '%W')
        vcmdForFloat2 = (self.mainFrame.register(self.validateFloat2), '%d', '%i', '%P', '%s', '%S', '%v', '%V', '%W')
        vcmdForHiddenLayerSizes = (
        self.mainFrame.register(self.validateHiddenLayerSizes), '%d', '%i', '%P', '%s', '%S', '%v', '%V', '%W')
    ...
    def compute(self):
        ...
        dict = {"Training Set Size": 1.00 - float(self.test_size.get()),
                "Testing Set Size": float(self.test_size.get()), "Training Set Shape": X_train.shape,
                "Testing Set Shape": X_test.shape, "Classifier": self.classifier,
